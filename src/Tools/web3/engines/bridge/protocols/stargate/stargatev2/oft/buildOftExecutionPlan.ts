...
export async function buildOftExecutionPlan(
  ...
  if (context.resolution.executionMode !== "v2_oft") {
    throw new Error("Resolved Stargate V2 route is not a simple OFT execution mode.");
  }
  ...
  const steps: BridgePlanStep[] = [
    {
      type: "bridge",
      tool: "write_contract",
      description: "Execute Stargate V2 send via direct OFT token",
      args: {
